...
public sealed class TalosForgeHubOverlayDrawingTests
{
    [Fact]
    public async Task DisabledOverlay_DoesNotSendCommand()
    {
        ...
        var options = new BotOptions
        {
            ...
            InGameOverlayEveryTicks = 5,
            InGameLuaQueryPollEveryTicks = 10_000,
        };
        var service = new TalosForgeHubOverlayService(unlocker, options);

        var count = await service.TryPublishAsync(
            5,
            BotState.Idle,
            BuildSnapshot(5),
            0,
    ...
    public async Task EnabledOverlay_SendsImmediately_When_First_Becoming_Visible_Then_Respects_Interval()
    {
        ...
        var c1 = await service.TryPublishAsync(
            2,
            BotState.Idle,
            BuildSnapshot(2),
            0,
            lastTickMetrics: null,
            CancellationToken.None);
        var c2 = await service.TryPublishAsync(
            3,
            BotState.Combat,
            BuildSnapshot(3),
            1,
        ...
        Assert.Contains("Tick", lua, StringComparison.Ordinal);
    }
