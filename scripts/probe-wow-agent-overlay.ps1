...
for ($i = 1; $i -le $Repeat; $i++) {
    ...
    $lua = "local f=_G['TalosForgeManualProbe']; if not f then f=CreateFrame('Frame','TalosForgeManualProbe',UIParent); f:SetSize(1000,80); f:SetPoint('TOP',UIParent,'TOP',0,-90); f:SetFrameStrata('TOOLTIP'); f:SetFrameLevel(9999); end; if not f.bg then local bg=f:CreateTexture(nil,'BACKGROUND'); bg:SetAllPoints(true); f.bg=bg; end; f.bg:SetTexture(0,0,0,0.7); if not f.t then local t=f:CreateFontString(nil,'OVERLAY'); t:SetAllPoints(true); t:SetJustifyH('CENTER'); if GameFontNormalHuge then t:SetFontObject(GameFontNormalHuge) elseif NumberFontNormalLarge then t:SetFontObject(NumberFontNormalLarge) elseif ChatFontNormal then t:SetFontObject(ChatFontNormal) end; t:SetTextColor(1,0.2,0.2,1); t:SetShadowOffset(2,-2); t:SetShadowColor(0,0,0,1); f.t=t; end; f.t:SetText('" + $safeText + "'); f:Show(); if UIErrorsFrame and UIErrorsFrame.AddMessage then UIErrorsFrame:AddMessage('" + $safeText + "',1,0.2,0.2,1); end;"
